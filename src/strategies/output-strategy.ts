...
import type { StrategyResult, ValidationResult } from "./shared/types.js";
...
export interface OutputStrategy<TDomainResult> {
	...
	/**
	 * Optional BaseStrategy-compatible validation hook.
	 *
	 * Strategies migrated to BaseStrategy can expose this lifecycle method
	 * while preserving compatibility with legacy render-based strategies.
	 *
	 * @param result - The domain result to validate
	 * @returns Validation outcome
	 */
	validate?(result: TDomainResult): ValidationResult;

	/**
	 * Optional BaseStrategy-compatible execution hook.
	 *
	 * @param result - The validated domain result
	 * @returns Output artifacts generated by the strategy
	 */
	execute?(result: TDomainResult): Promise<OutputArtifacts>;

	/**
	 * Optional BaseStrategy-compatible run method.
	 *
	 * @param result - The strategy input
	 * @returns Structured strategy result including success/failure metadata
	 */
	run?(result: TDomainResult): Promise<StrategyResult<OutputArtifacts>>;
}
