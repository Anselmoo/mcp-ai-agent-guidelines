 ...
 *
 * @extends {BaseStrategy<ADRSessionState | PromptResult, OutputArtifacts>}
 */
export class ADRStrategy extends BaseStrategy<
	ADRSessionState | PromptResult,
	OutputArtifacts
> {
	protected readonly name = "adr";
	 ...
	 */
	validate(input: ADRSessionState | PromptResult): ValidationResult {
			...
			errors: [
				{
					...
					message: "Input must be ADRSessionState or PromptResult",
				},
			],
			warnings: [],
		};
	...
	/**
	 * Execute the ADR rendering strategy.
	 *
	 * @param input - The domain result to render
	 * @returns Output artifacts with primary ADR document
	 */
	async execute(
		input: ADRSessionState | PromptResult,
		options?: Partial<RenderOptions>,
	): Promise<OutputArtifacts> {
		if (this.isSessionState(input)) {
			return this.renderSessionState(input);
		}
		return this.renderPromptResult(input as PromptResult, options);
	}
...
---
*ADR generated by hierarchical-prompt-builder*${metadataFooter}
`;
