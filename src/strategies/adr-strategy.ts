...
/**
 * SessionState interface for design workflow sessions.
 * Minimal subset needed for ADR generation.
 */
 ...
 *
 * Supports rendering:
 * - SessionState: Design workflow decisions
	...
	implements OutputStrategy<SessionState | PromptResult>
{
	...
	/**
	 * Render a SessionState to ADR format.
	 *
...
`;
	...
	/**
	 * Extract context from SessionState.
	 *
	 ...
	 * @private
	 */
	private extractContext(result: SessionState): string {
		const parts: string[] = [];
	...
	/**
	 * Extract decision statement from SessionState.
	 *
	 ...
	 * @private
	 */
	private extractDecision(result: SessionState): string {
	...
	/**
	 * Extract positive consequences from SessionState.
	 *
	 ...
	 * @private
	 */
	private extractPositiveConsequences(result: SessionState): string {
		const consequences =
	...
	/**
	 * Extract negative consequences from SessionState.
	 *
	 ...
	 * @private
	 */
	private extractNegativeConsequences(result: SessionState): string {
		const consequences =
	...
	/**
	 * Extract neutral consequences from SessionState.
	 *
	 ...
	 * @private
	 */
	private extractNeutralConsequences(result: SessionState): string {
		const consequences =
	...
	/**
	 * Extract references from SessionState.
	 *
	...
	private isSessionState(result: unknown): result is SessionState {
		return (
			...
			result !== null &&
			"id" in result &&
			typeof (result as SessionState).id === "string"
		);
	}
