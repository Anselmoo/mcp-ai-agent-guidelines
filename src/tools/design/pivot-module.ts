...
import { z } from "zod";
import type { PivotDecision, DesignSessionState } from "./types.js";
...
class PivotModuleImpl {
	...
	async evaluatePivotNeed(request: PivotRequest): Promise<PivotDecision> {
		...
		const alternatives = triggered ? await this.generateAlternatives(sessionState, complexityScore, entropyLevel) : [];
	...
	private async calculateComplexityScore(sessionState: DesignSessionState, content: string): Promise<number> {
	...
	private async measureEntropyLevel(sessionState: DesignSessionState, content: string): Promise<number> {
	...
	private analyzeComplexityFactors(sessionState: DesignSessionState, content: string): ComplexityFactors {
		const contentLower = content.toLowerCase();
		
		// Technical complexity indicators
		const technicalKeywords = ['api', 'database', 'microservice', 'integration', 'protocol', 'algorithm'];
		...
		const businessKeywords = ['stakeholder', 'compliance', 'regulation', 'process', 'workflow', 'approval'];
		...
		const integrationKeywords = ['external', 'third-party', 'legacy', 'migration', 'sync', 'federation'];
		...
		const userKeywords = ['interface', 'experience', 'personalization', 'accessibility', 'localization'];
		const userComplexity = this.calculateKeywordComplexity(contentLower, userKeywords, 10);

		// Maintenance complexity indicators
		const maintenanceKeywords = ['monitoring', 'logging', 'deployment', 'scaling', 'backup', 'security'];
	...
	private analyzeEntropyFactors(sessionState: DesignSessionState, content: string): EntropyFactors {
		const contentLower = content.toLowerCase();
		
		// Uncertainty indicators
		const uncertaintyKeywords = ['unclear', 'unknown', 'tbd', 'pending', 'investigate', 'research'];
		const requirementUncertainty = this.calculateKeywordComplexity(contentLower, uncertaintyKeywords, 30);

		const technicalUncertaintyKeywords = ['prototype', 'experiment', 'proof of concept', 'feasibility', 'spike'];
		const technicalUncertainty = this.calculateKeywordComplexity(contentLower, technicalUncertaintyKeywords, 25);

		const timelineUncertaintyKeywords = ['estimate', 'roughly', 'approximately', 'depends', 'variable'];
		const timelineUncertainty = this.calculateKeywordComplexity(contentLower, timelineUncertaintyKeywords, 20);

		const resourceUncertaintyKeywords = ['resource', 'capacity', 'availability', 'allocation', 'constraint'];
		...
		const conflictKeywords = ['disagree', 'conflict', 'dispute', 'concern', 'objection', 'blocker'];
	...
	private calculateKeywordComplexity(content: string, keywords: string[], baseScore: number): number {
		let matches = 0;
	...
	private generateRecommendation(complexity: number, entropy: number, alternatives: string[]): string {
	...
	async identifyBottlenecks(sessionState: DesignSessionState): Promise<string[]> {
		const bottlenecks: string[] = [];
		
		// Analyze session state for bottlenecks
		const blockedPhases = Object.values(sessionState.phases).filter(p => p.status === 'blocked');
		if (blockedPhases.length > 0) {
			bottlenecks.push(`Blocked phases: ${blockedPhases.map(p => p.name).join(', ')}`);
		}

		const overduePhasesCount = Object.values(sessionState.phases).filter(p => 
			p.status === 'in-progress' && p.coverage < 50
		).length;
	...
	async recommendSimplification(
		...
		if (complexity > 90) {
			recommendations.push("Consider a radical simplification - reduce to core MVP features only");
			recommendations.push("Replace complex custom solutions with proven third-party tools");
		}

		if (complexity > 80) {
			recommendations.push("Break the system into smaller, independent modules");
