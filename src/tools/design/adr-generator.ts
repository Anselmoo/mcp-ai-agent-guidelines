...
const ADRRequestSchema = z.object({
	...
	status: z.enum(['proposed', 'accepted', 'deprecated', 'superseded']).optional().default('proposed'),
...
class ADRGeneratorImpl {
	...
	async generateADR(request: ADRRequest): Promise<ADRGenerationResult> {
		const { sessionState, title, context, decision, consequences, alternatives, status, metadata } = request;
		...
		const adrFormat = constraintManager.getOutputFormatSpec('adr');
		...
		const markdown = this.generateADRMarkdown({
			...
			context,
			decision,
			consequences: consequences || this.generateConsequences(decision, sessionState),
...
${adr.alternatives.length > 0 ? `## Alternatives Considered

${adr.alternatives.map((alt, index) => `### Alternative ${index + 1}
...
` : ''}`;
	...
	private generateConsequences(decision: string, sessionState: DesignSessionState): string {
		...
		if (decisionLower.includes('microservice') || decisionLower.includes('distributed')) {
			consequences.push("**Technical**: Increased system complexity and network latency");
			consequences.push("**Operational**: Requires advanced monitoring and deployment strategies");
		}
		
		if (decisionLower.includes('monolith') || decisionLower.includes('single')) {
			consequences.push("**Technical**: Simpler deployment but potential scaling limitations");
			consequences.push("**Development**: Easier local development and testing");
		}
		
		if (decisionLower.includes('cloud') || decisionLower.includes('aws') || decisionLower.includes('azure')) {
		...
		if (decisionLower.includes('database') || decisionLower.includes('storage')) {
			consequences.push("**Data**: Data consistency and backup strategies required");
			consequences.push("**Performance**: Query optimization and indexing needed");
		}
		
		// Business consequences
		if (decisionLower.includes('compliance') || decisionLower.includes('security')) {
			consequences.push("**Compliance**: Additional audit and certification requirements");
			consequences.push("**Risk**: Enhanced security posture but implementation overhead");
		}
		
		// Default consequences if none detected
		if (consequences.length === 0) {
			consequences.push("**Positive**: Addresses identified requirements and constraints");
			consequences.push("**Negative**: Requires implementation effort and potential technical debt");
			consequences.push("**Neutral**: Should be monitored for long-term impact");
		}
	...
	private generateADRRecommendations(request: ADRRequest): string[] {
		...
		const { decision, sessionState } = request;
		
		// Check against constraints
		const validation = constraintManager.validateConstraints(decision);
		if (!validation.passed) {
			recommendations.push("Address constraint violations before accepting this ADR");
			recommendations.push(...validation.recommendations.slice(0, 2));
		}
		
		// General ADR recommendations
		recommendations.push("Review ADR with relevant stakeholders before implementation");
		recommendations.push("Update related documentation and architecture diagrams");
		recommendations.push("Plan implementation timeline and resource allocation");
		
		// Context-specific recommendations
		if (decision.toLowerCase().includes('new technology')) {
			recommendations.push("Conduct proof-of-concept before full implementation");
			recommendations.push("Plan team training and knowledge transfer");
		}
		
		if (decision.toLowerCase().includes('security') || decision.toLowerCase().includes('auth')) {
	...
	private findRelatedDecisions(sessionState: DesignSessionState, decision: string): string[] {
	...
	private extractKeywords(text: string): string[] {
		...
		const techTerms = [
			'api', 'database', 'microservice', 'monolith', 'cloud', 'aws', 'azure', 'gcp',
			'security', 'auth', 'oauth', 'jwt', 'ssl', 'https', 'encryption',
			'docker', 'kubernetes', 'container', 'serverless', 'lambda',
	...
	async generateSessionADRs(sessionState: DesignSessionState): Promise<Artifact[]> {
		const adrs: Artifact[] = [];
	...
	private extractDecisionsFromPhase(phase: any): Array<{title: string; content: string}> {
		...
		for (const artifact of phase.artifacts) {
			...
			for (const pattern of decisionPatterns) {
				...
				for (const match of matches) {
					...
					if (matchResult && matchResult[1] && typeof matchResult[1] === 'string' && matchResult[1].trim().length > 10) {
						decisions.push({
							title: matchResult[1].trim().slice(0, 50) + (matchResult[1].length > 50 ? '...' : ''),
