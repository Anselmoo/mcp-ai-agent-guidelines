...
test("mermaidDiagramGenerator attempts repair when validation fails and repair=true", async () => {
	...
	(HANDLER_REGISTRY as any).flowchart = {
		generate: () => "flowchart node A --> B", // missing header (no 'flowchart TD' at start)
	} as any;
	...
	await withMermaidProvider(
		...
		async () => {
			...
			const text = resp.content[0].text as string;
			const isRepaired = text.includes("(after auto-repair)");
			const isInvalidAfterAttempts = text.includes(
				"Diagram invalid even after attempts",
			);
			expect(isRepaired || isInvalidAfterAttempts).toBe(true);
			// If repaired, the header should be present
			if (isRepaired) {
