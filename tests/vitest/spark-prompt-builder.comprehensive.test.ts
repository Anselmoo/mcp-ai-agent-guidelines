...
describe("spark-prompt-builder comprehensive coverage", () => {
	it("should handle complete configuration with all optional fields", async () => {
		const result = await sparkPromptBuilder({
			...
			edgeCases: [
				{ name: "Network failure", handling: "Show offline mode with cached data" },
				{ name: "Large datasets", handling: "Implement pagination and virtualization" }
			],
			...
			secondaryColors: [
				{ name: "Success Green", oklch: "oklch(65% 0.15 120)", usage: "Success states and confirmations" },
				{ name: "Warning Orange", oklch: "oklch(70% 0.15 60)", usage: "Warning messages and alerts" },
				{ name: "Error Red", oklch: "oklch(55% 0.15 15)", usage: "Error states and validation" }
			],
			...
			fontReasoning: "Inter provides excellent readability at small sizes and has extensive character support",
			typography: [
				{ usage: "Headlines", font: "Inter", weight: "600", size: "2rem", spacing: "-0.02em" },
				{ usage: "Body text", font: "Inter", weight: "400", size: "1rem", spacing: "0em" },
				{ usage: "Captions", font: "Inter", weight: "500", size: "0.875rem", spacing: "0.01em" },
				{ usage: "Code", font: "JetBrains Mono", weight: "400", size: "0.875rem", spacing: "0em" }
			],
			animationPhilosophy: "Subtle and purposeful motion that enhances UX",
			animationRestraint: "Respect prefers-reduced-motion and keep durations under 300ms",
			animationPurpose: "Guide attention, provide feedback, and improve perceived performance",
			animationHierarchy: "Primary actions > secondary actions > decorative elements",
			components: [
				{
					type: "Button",
			...
			mobileLayout: "Single column with collapsible sections and bottom navigation",
			mode: "design-system",
	...
	it("should handle all animation and color variations", async () => {
		const result = await sparkPromptBuilder({
			...
			secondaryColors: [
				{ name: "Info Blue", oklch: "oklch(60% 0.15 240)", usage: "Information states" },
				{ name: "Neutral Gray", oklch: "oklch(50% 0.02 240)", usage: "Secondary text" }
			],
			foregroundBackgroundPairings: [
				{
