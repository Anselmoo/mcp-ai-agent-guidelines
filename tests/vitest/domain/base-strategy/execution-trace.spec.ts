...
describe("domain/base-strategy/execution-trace", () => {
	...
	describe("recordMetric", () => {
		...
		it("should handle floating point values", () => {
			// Arrange & Act
			trace.recordMetric("float_value", 3.14159);

			// Assert
			expect(trace.metrics.float_value).toBe(3.14159);
		});
	});
	...
	describe("trace-utils", () => {
		describe("createTrace", () => {
			...
			it("should create trace with unique execution ID", () => {
				...
				// Assert
				expect(trace1.toData().executionId).not.toBe(
					trace2.toData().executionId,
				);
			});
		});
		...
		describe("withErrorTracing", () => {
			...
			it("should record error with context", async () => {
				...
				// Act & Assert
				await expect(
					withErrorTracing(trace, operation, context),
