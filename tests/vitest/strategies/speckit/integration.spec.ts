...
describe("Spec-Kit Integration", () => {
	describe("End-to-end generation", () => {
		it("generates all 7 artifacts (README + 6 secondary)", () => {
			...
			expect(secondaryNames.some((name) => name.includes("spec.md"))).toBe(
				true,
			);
			expect(secondaryNames.some((name) => name.includes("plan.md"))).toBe(
				true,
			);
			expect(secondaryNames.some((name) => name.includes("tasks.md"))).toBe(
				true,
			);
			expect(secondaryNames.some((name) => name.includes("progress.md"))).toBe(
				true,
			);
			expect(secondaryNames.some((name) => name.includes("adr.md"))).toBe(true);
			expect(secondaryNames.some((name) => name.includes("roadmap.md"))).toBe(
				true,
			);
		});
