...
The `gap-frameworks-analyzers` capability, performance, maturity, skills, technology gaps.
...
```json
{
  ...
  "currentState": "your-current-state-here",
  "desiredState": "your-desired-state-here",
...
| `currentState` | string | ✅ Yes | - | Description of current state for gap analysis |
| `desiredState` | string | ✅ Yes | - | Description of desired future state |
...
| `timeframe` | string | No | - | Analysis timeframe (e.g., '6 months', '1 year') |
| `includeActionPlan` | boolean | No | `false` | Generate actionable implementation plan |
...
1. **Current** - Current vs desired state analysis
2. **12** - 12 framework types (capability, performance, maturity, skills, technology)
3. **Action** - Action plan generation
4. **Stakeholder** - Stakeholder impact assessment

### Output Structure

```markdown
## Gap Frameworks Analyzers Output
...
```json
{
  ...
  "currentState": "Example currentState value for strategic planning session",
  "desiredState": "Example desiredState value for strategic planning session",
...
```mermaid
graph LR
  A[gap-frameworks-analyzers] --> B[strategy-frameworks-builder]
  B --> C[sprint-timeline-calculator]
  C --> D[Execute/Apply]
```

1. **gap-frameworks-analyzers** - Gap analysis frameworks
